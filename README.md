...
- Not on the live roster → `200` `{ "status": "skipped", "reason": "not_listed" }`, except an unlisted address whose `GET /invoices/eligible` `status` is `admitted` or `trial` is paid **1 USD** (new-member handbook cap) using the daily list comment. Lookup failure / `GiftsApiError` on that GET → `200` skipped `eligible_unreachable` (no JSONL, no pay). Other unlisted statuses stay `not_listed`.
