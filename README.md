...
- Only Lightning Addresses that currently have a passkey on 21.gifts are paid (live `GET /invoices/passkey` each run). Addresses without a passkey (or without an account) are skipped as `no_passkey` with no JSONL row so catch-up can retry later the same UTC day; a failed passkey lookup aborts the whole run with exit `3` (`passkey_unreachable`) and pays no one
