...
- `uncertain` is only after an invoice id / pay attempt (amount mismatch, lndhub.pay error, missing/mismatched preimage, proof failure). Invoice-create parse failures (`malformed invoice response` / `malformed paymentHash`) are also `uncertain` + halt, without a pay. Those still halt the rest of a live run (`--live` or `SPEND_LIVE=true`), append a `*halt*` JSONL row, `markFinished`, exit `4`, and are **not** retried the same UTC day
